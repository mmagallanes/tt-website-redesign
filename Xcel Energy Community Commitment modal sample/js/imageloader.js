...
$(document).ready(function() { 
    ...
    function tryImage() {
      ...
      if (complete && img.complete) {
        if (img.width && img.height) {
          callback({ img: img.src, img_id: options.id, img_class: options.img_class });
          done = true;
          return;
        }
        callback({ err: '404' });
  ...
  while (i <= max) {
    ...
    loadImage({ src : image_url, id : image_id, img_class: isactive, maxSeconds : 30 }, function(status) {
        if(status.err) {
